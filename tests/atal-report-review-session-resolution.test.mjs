...
test('report.review recovers a fabricated session id only when context has exactly one completed session', () => {
  ...
  assert.equal(result.entities.session.id, 'session-canonical');
});
