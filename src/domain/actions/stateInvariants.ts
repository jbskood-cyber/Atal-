...
function fail(message: string): never {
  throw actionError('CORE_INVARIANT_FAILED', message);
}
...
export function validateActionStateInvariants(candidate: AtalState, previous?: AtalState): void {
  ...
  for (const name of collectionNames) assertUniqueIds(candidate[name] as Array<{ id: string }>, name);
