...
export function ClinicalRecordScreen({ patientId }: { patientId: string }) {
  ...
  const dirty = editing && Boolean(baseline) && JSON.stringify(record) !== baseline;
  ...
  useEffect(() => {
    if (stored && !editing) setRecord(stored);
  ...
  const save = () => {
    ...
    if (current.painLevel !== null && (current.painLevel < 0 || current.painLevel > 10)) { setMessage('El dolor debe estar entre 0 y 10.'); return; }
    try {
      let saved: ClinicalRecord;
      if (record && record.id !== 'pending-record') saved = updateClinicalRecord(record.id, record) ?? record;
      else {
