...
export function ConversationalDraftCard({ draft,patientLabel,applying,applied,conflict,onChange,onApply,onReviewAll,onRefreshConflict,onCompare,onKeepVersion }: { draft:AtalAIDraft;patientLabel:string;applying:boolean;applied:boolean;conflict?:string;onChange:(draft:AtalAIDraft)=>void;onApply:()=>void;onReviewAll:()=>void;onRefreshConflict:()=>void;onCompare:()=>void;onKeepVersion:()=>void }) {
  ...
  return <>
    <section className="atal-draft-card" aria-label="Borrador preparado">
      <button type="button" className="atal-draft-card-header" aria-expanded={cardOpen} onClick={()=>{setCardOpen((value)=>!value);setOpenSection(null)}}><span><small>Borrador editable</small><b>{draft.responseMode==='command'?'Acción preparada':'Borrador del plan de tratamiento'}</b></span><Status percent={total} review={warnings>0}/><ChevronDown className={cardOpen?'is-open':''}/></button>
      ...
      {cardOpen && <footer className={draft.responseMode==='command'?'is-single-action':''}>{draft.responseMode!=='command'&&<button type="button" onClick={onReviewAll}>Revisar todo</button>}<button type="button" className="is-primary" disabled={applying||applied||Boolean(conflict)} onClick={onApply}><Save/>{applied?'Aplicado a Atal':applying?'Aplicando…':'Aplicar cambios'}</button></footer>}
    </section>
...
function SectionPreview({section,draft}:{section:DraftSectionId;draft:AtalAIDraft}) {
  ...
  if(section==='record')return <dl><div><dt>Evolución</dt><dd>{draft.patient.evolutionTime||'Por completar'}</dd></div><div><dt>Hallazgos</dt><dd>{draft.patient.clinicalNotes||draft.patient.providedDiagnosis||'Por completar'}</dd></div><div><dt>Precauciones</dt><dd>{draft.patient.precautions.join(', ')||'Sin precauciones indicadas'}</dd></div></dl>;
  if(section==='plan')return <dl><div><dt>Objetivo general</dt><dd>{draft.plan.goal||'Por completar'}</dd></div><div><dt>Enfoque clínico</dt><dd>{draft.plan.focus||'Por completar'}</dd></div><div><dt>Duración y frecuencia</dt><dd>{duration(draft)} · {frequency(draft)}</dd></div><div><dt>Criterios de progreso</dt><dd>{draft.plan.progressCriteria||'Por completar'}</dd></div></dl>;
  if(section==='exercises')return <ol>{draft.exercises.map((item)=><li key={item.id}><b>{item.name||'Sin nombre'}</b><span>{item.sets??'—'} series · {item.repetitions||item.duration||'dosis por completar'}</span></li>)}</ol>;
  if(section==='note')return <p>{draft.command?.content||'Nota por completar'}</p>;
  return <p>{draft.assistantMessage||draft.command?.content||'Acción preparada para revisión.'}</p>;
}
