...
import { useModalIsolation } from '@/src/components/atal/useModalIsolation';
...
export function AttachmentMenu({ open, onClose, onFiles }: { open: boolean; onClose: () => void; onFiles: (files: FileList) => void }) {
  ...
  return <>
    ...
    {open && <div className="atal-ai-attachment-backdrop" onMouseDown={onClose}><section className="atal-ai-attachment-menu" role="dialog" aria-modal="true" aria-label="Adjuntar a Atal IA" onMouseDown={(event) => event.stopPropagation()}>
      <header><h2>Adjuntar a Atal IA</h2><button ref={closeRef} type="button" aria-label="Cerrar" onClick={onClose}><X /></button></header>
      <button type="button" onClick={() => pick(cameraRef)}><span><Camera /></span><b>Cámara</b></button>
      <button type="button" onClick={() => pick(photosRef)}><span><Images /></span><b>Fotos</b></button>
    </section></div>}
  </>;
}
