...
export function AtalAIDraftReviewScreen({ draftId }: { draftId: string }) {
  ...
  if (!draft || !conversation) return <main className="atal-content atal-flow-page atal-ai-exclusive-review"><div className="atal-panel-placeholder"><FileText /><h1>Borrador no encontrado</h1><p>El borrador pudo haber sido descartado.</p><button type="button" onClick={() => router.push('/assistant')}>Volver a Atal IA</button></div></main>;
  ...
  const apply = () => {
    ...
    try {
      const savedResult = applyAtalAIDraft(draft, contact, { conversationId:conversation.id,draftId:draft.id });
  ...
  return <main className="atal-content atal-flow-page atal-ai-review-page atal-ai-exclusive-review">
