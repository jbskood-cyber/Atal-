...
const executeClientEffect=(clientEffect:ClientEffect)=>{if(clientEffect.type!=='download')return;const blob=new Blob([clientEffect.content],{type:clientEffect.mimeType});const url=URL.createObjectURL(blob);const anchor=document.createElement('a');anchor.href=url;anchor.download=clientEffect.filename;anchor.click();window.setTimeout(()=>URL.revokeObjectURL(url),1000)};
...
export function AtalAIConversationScreen() {
  ...
  const [pendingConfirmation,setPendingConfirmation]=useState<{decision:PolicyDecision;invocation:ToolInvocation}|null>(null);
  ...
  const process=async(request:AtalAIAnalyzeRequest,userMessage?:AIMessage)=>{
    ...
    try{
      ...
      if(next.responseMode==='query'&&next.command){const coreResult=executeLegacyAIAction({command:next.command,workContext:conversation.workContext,metadata:{conversationId:conversation.id,draftId:next.id}});if(coreResult.status==='success'){append({id:uid('message'),role:'assistant',text:coreResult.message,createdAt:new Date().toISOString(),attachments:[]});patchConversation({status:draft?'ready_for_review':'empty'});}else if(coreResult.status==='clarification'){append({id:uid('message'),role:'assistant',text:coreResult.clarification.message,createdAt:new Date().toISOString(),attachments:[]});patchConversation({status:draft?'ready_for_review':'empty'});}else if(coreResult.status==='confirmation-required'){setDraft(next);setPendingConfirmation(coreResult);setConfirm('command');patchConversation({status:'ready_for_review'});}else if(coreResult.status==='blocked'){patchConversation({status:'error',error:coreResult.message});}else if(coreResult.status==='error'){patchConversation({status:'error',error:coreResult.message});}}
  ...
  const apply=()=>{if(draft)void performApply()};
  const performApply=async(confirmation?:ConfirmationProof)=>{if(!draft)return;setApplying(true);try{const coreResult=executeLegacyAIAction({draft:draft.responseMode==='command'?undefined:draft,command:draft.responseMode==='command'?draft.command??undefined:undefined,privateContact:conversation.privateContact,workContext:conversation.workContext,metadata:{conversationId:conversation.id,draftId:draft.id,force:forceApply},confirmation});if(coreResult.status==='clarification'){setNotice(coreResult.clarification.message);return}if(coreResult.status==='confirmation-required'){setPendingConfirmation(coreResult);setConfirm('command');return}if(coreResult.status==='blocked'){setNotice(coreResult.message);return}if(coreResult.status==='error'){setNotice(coreResult.message);return}if(coreResult.status==='success'){if(coreResult.clientEffect)executeClientEffect(coreResult.clientEffect);const data=(coreResult.data??{}) as {patientId?:string;planId?:string;clinicalRecordId?:string;exerciseId?:string};const result={...data,summary:coreResult.summary,undo:coreResult.undo,planId:(data.planId??draft.command?.planId??conversation.workContext.selectedPlanId)||undefined,patientId:(data.patientId??draft.command?.patientId??conversation.workContext.selectedPatientId)||undefined};const next={...conversation,status:'saved' as const,savedResult:result,updatedAt:new Date().toISOString()};setConversation(next);saveAIConversation(next);append({id:uid('message'),role:'assistant',text:`Cambios aplicados. ${result.summary.join(' ')}`,createdAt:new Date().toISOString(),attachments:[]});setPendingConfirmation(null);setConfirm(null);setForceApply(false)}}catch(error){setNotice(error instanceof Error?error.message:'No pudimos aplicar los cambios.')}finally{setApplying(false)}};
  const confirmExecution=()=>{if(!pendingConfirmation)return;const mode=pendingConfirmation.decision.mode;if(mode!=='review'&&mode!=='explicit'&&mode!=='reinforced'){setNotice('Esta acción no puede confirmarse.');return}const now=new Date().toISOString();void performApply({id:uid('confirmation'),fingerprint:pendingConfirmation.decision.fingerprint,mode:pendingConfirmation.decision.mode as 'review'|'explicit'|'reinforced',confirmedAt:now,expiresAt:new Date(Date.parse(now)+5*60_000).toISOString()})};
  const undo=()=>{const receipt=conversation.savedResult?.undo;if(!receipt)return;if(!('patches' in receipt)){setNotice('Este cambio pertenece a una versión anterior y ya no puede deshacerse de forma segura.');return}try{executeUndo(receipt,executionContext(conversation.workContext,{conversationId:conversation.id,draftId:conversation.draftId}));patchConversation({savedResult:{...conversation.savedResult!,undo:undefined}});append({id:uid('message'),role:'assistant',text:'Cambio deshecho correctamente.',createdAt:new Date().toISOString(),attachments:[]})}catch(error){setNotice(error instanceof Error?error.message:'No fue posible deshacer.')}};
  ...
  return <main className="atal-command-page">
    ...
    {confirm&&<ConfirmDialog
      kind={confirm}
      draft={draft}
      onCancel={()=>{setConfirm(null);setPendingConfirmation(null)}}
      onConfirm={()=>confirm==='discard'?discard():confirm==='restart'?restart():confirmExecution()}
    />}
