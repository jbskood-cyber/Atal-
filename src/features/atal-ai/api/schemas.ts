...
export const atalAIDraftJsonSchema = {
  ...
  properties: {
    intent: { type: 'string', enum: ['create_patient_plan'] },
    ...
    missingFields: stringArray, uncertainFields: stringArray, contradictions: stringArray, followUpQuestion: { type: 'string' },
  },
  required: ['intent','patient','plan','exercises','missingFields','uncertainFields','contradictions','followUpQuestion'],
} as const;
...
export function normalizeAtalAIDraft(value: unknown, existingId?: string): AtalAIDraft {
  ...
  return {
    id: existingId ?? localId('ai-draft'), intent: 'create_patient_plan',
    ...
    exercises: exerciseDrafts, missingFields: list(source.missingFields), uncertainFields: list(source.uncertainFields), contradictions: list(source.contradictions), followUpQuestion: text(source.followUpQuestion), createdAt: now, updatedAt: now,
  };
}
