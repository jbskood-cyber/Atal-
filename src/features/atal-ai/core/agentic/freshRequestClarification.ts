 ...
 * contain the missing change. This intentionally avoids asking Gemini to infer
 * a mutation from an underspecified command.
 */
export function freshRequestClarification(text: string, hasConversationContext: boolean): string | undefined {
  if (hasConversationContext || !isFreshUnderspecifiedPlanMutation(text)) return undefined;
