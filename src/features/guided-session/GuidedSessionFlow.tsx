...
export function GuidedSessionFlow({ patientId }: { patientId: string }) {
  ...
  const restart = () => {
    ...
    setDraft(createSessionDraft(patientId, plan.id, plan.exercises));
    setResumeGate(false);
  ...
  const finish = () => {
    ...
    const next: GuidedSessionDraft = { ...draft, status: allComplete ? 'completed' : 'partial', stage: 'summary', completedAt: new Date().toISOString() };
    saveCompletedSession(patientId, plan.id, next);
    clearSessionDraft(patientId, plan.id);
    setDraft(next);
  };
  ...
  return <PatientSessionFrame label={draft.stage === 'exercise' ? `Ejercicio ${currentIndex + 1} de ${plan.exercises.length}` : draft.stage === 'summary' ? 'Resumen' : draft.stage === 'close' ? 'Cierre' : 'Preparación'} progress={progress} onExit={() => router.push(`/patients/${patientId}/portal-preview`)}>
    {resumeGate ? <section className="atal-session-card atal-resume-card"><RotateCcw /><span className="atal-session-kicker">Sesión sin terminar</span><h1>Continúa donde lo dejaste</h1><p>Conservamos el ejercicio actual, tus series, valores y comentarios.</p><div><button type="button" className="atal-session-primary" onClick={() => setResumeGate(false)}>Continuar sesión</button><button type="button" onClick={() => setConfirmRestart(true)}>Empezar de nuevo</button></div></section> : <>
