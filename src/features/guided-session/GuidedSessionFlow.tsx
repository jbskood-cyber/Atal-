...
export function GuidedSessionFlow({ patientId }: { patientId: string }) {
  ...
  const startSession = () => { const startedAt = draft.startedAt ?? new Date().toISOString(); recordSessionStarted(patientId, plan.id, startedAt); setDraft((current) => ({ ...current, stage: 'exercise', status: 'in_progress', startedAt })); };
  ...
  return <PatientSessionFrame planName={plan.name} label={draft.stage === 'exercise' ? `Ejercicio ${currentIndex + 1} de ${plan.exercises.length}` : draft.stage === 'summary' ? 'Resumen' : draft.stage === 'close' ? 'Cierre' : 'Preparación'} progress={progress} onExit={() => router.push(`/patients/${patientId}/portal-preview`)}>
    {resumeGate ? <section className="atal-session-card atal-resume-card"><RotateCcw /><span className="atal-session-kicker">Sesión sin terminar</span><h1>Continúa donde lo dejaste</h1><p>Conservamos el ejercicio actual, tus series, valores y comentarios en este dispositivo.</p><div><button type="button" className="atal-session-primary" onClick={() => setResumeGate(false)}>Continuar sesión</button><button type="button" onClick={() => setConfirmRestart(true)}>Empezar de nuevo</button></div></section> : <>
      {draft.stage === 'prepare' && <SessionPreparation plan={plan} draft={draft} onChange={setDraft} onStart={startSession} />}
