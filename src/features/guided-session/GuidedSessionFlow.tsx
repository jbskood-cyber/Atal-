...
import { guidedDemoPlan } from './demo';
...
export function GuidedSessionFlow({ patientId }: { patientId: string }) {
  ...
  const restored = patient ? readSessionDraft(patientId) : { draft: null, error: false };
  const [draft, setDraft] = useState<GuidedSessionDraft>(() => restored.draft ?? createSessionDraft(patientId, guidedDemoPlan.exercises));
  ...
  if (!guidedDemoPlan.exercises.length) return <PatientSessionFrame label="Plan sin ejercicios" progress={0} onExit={() => router.push(`/patients/${patientId}/portal-preview`)}><StateCard icon={<AlertTriangle />} title="Plan sin ejercicios" text="Tu fisioterapeuta todavía no ha añadido ejercicios a este plan." action="Volver al plan" onAction={() => router.push(`/patients/${patientId}/portal-preview`)} /></PatientSessionFrame>;

  const currentIndex = Math.min(Math.max(0, draft.currentExerciseIndex), guidedDemoPlan.exercises.length - 1);
  const exercise = guidedDemoPlan.exercises[currentIndex];
  const safeRecord = exercise ? draft.exercises[exercise.id] ?? createSessionDraft(patientId, guidedDemoPlan.exercises).exercises[exercise.id] : undefined;
  const completedCount = Object.values(draft.exercises).filter((record) => record.result).length;
  const progress = draft.stage === 'prepare' ? 4 : draft.stage === 'exercise' ? ((currentIndex + 0.2) / guidedDemoPlan.exercises.length) * 80 + 8 : draft.stage === 'close' ? 92 : 100;
  const restart = () => { clearSessionDraft(patientId); setDraft(createSessionDraft(patientId, guidedDemoPlan.exercises)); setResumeGate(false); setConfirmRestart(false); };
  ...
  return <PatientSessionFrame label={draft.stage === 'exercise' ? `Ejercicio ${currentIndex + 1} de ${guidedDemoPlan.exercises.length}` : draft.stage === 'summary' ? 'Resumen' : draft.stage === 'close' ? 'Cierre' : 'Preparación'} progress={progress} onExit={() => router.push(`/patients/${patientId}/portal-preview`)}>
    {resumeGate ? <section className="atal-session-card atal-resume-card"><RotateCcw /><span className="atal-session-kicker">Sesión sin terminar</span><h1>Continúa donde lo dejaste</h1><p>Conservamos el ejercicio actual, tus series, valores y comentarios en este dispositivo.</p><div><button type="button" className="atal-session-primary" onClick={() => setResumeGate(false)}>Continuar sesión</button><button type="button" onClick={() => setConfirmRestart(true)}>Empezar de nuevo</button></div></section> : <>
      {draft.stage === 'prepare' && <SessionPreparation plan={guidedDemoPlan} draft={draft} onChange={setDraft} onStart={() => setDraft((current) => ({ ...current, stage: 'exercise', status: 'in_progress', startedAt: current.startedAt ?? new Date().toISOString() }))} />}
      {draft.stage === 'exercise' && exercise && safeRecord && <ActiveExercise exercise={exercise} record={safeRecord} index={currentIndex} total={guidedDemoPlan.exercises.length} onChange={(record) => setDraft((current) => ({ ...current, currentExerciseIndex: currentIndex, exercises: { ...current.exercises, [exercise.id]: record } }))} onPrevious={() => setDraft((current) => ({ ...current, currentExerciseIndex: Math.max(0, currentIndex - 1) }))} onNext={() => setDraft((current) => currentIndex === guidedDemoPlan.exercises.length - 1 ? { ...current, currentExerciseIndex: currentIndex, stage: 'close' } : { ...current, currentExerciseIndex: currentIndex + 1 })} onFinishEarly={() => setConfirmFinish(true)} />}
      {draft.stage === 'close' && <SessionClose plan={guidedDemoPlan} draft={draft} onChange={setDraft} onFinish={finish} />}
      {draft.stage === 'summary' && <SessionSummary plan={guidedDemoPlan} draft={draft} onPlan={() => router.push(`/patients/${patientId}/portal-preview`)} onRestart={() => setConfirmRestart(true)} />}
    </>}
    {confirmFinish && <ConfirmDialog title="¿Terminar la sesión?" text={`Todavía quedan ${guidedDemoPlan.exercises.length - completedCount} ejercicios pendientes. La sesión se guardará como parcial.`} confirm="Terminar de todas formas" cancel="Continuar sesión" onConfirm={finishEarly} onCancel={() => setConfirmFinish(false)} />}
