...
export function PatientPortalPreviewScreen({ patientId }: { patientId: string }) {
  ...
  const restart = () => {
    clearSessionDraft(patientId, plan.id);
    setRestartOpen(false);
  ...
  return <main className="atal-patient-plan">
    ...
    <section className="atal-patient-plan-card">
      ...
      {canResume && plan.status === 'active' ? <div className="atal-plan-session-actions"><button type="button" className="atal-session-primary" onClick={() => router.push(`/patients/${patientId}/session`)}><Play /> Continuar donde lo dejaste</button><button ref={restartTriggerRef} type="button" onClick={() => setRestartOpen(true)}>Empezar de nuevo</button></div> : <button type="button" className="atal-session-primary atal-start-session" disabled={!plan.exercises.length || plan.status !== 'active'} onClick={() => { if (hasCompletedSession) clearSessionDraft(patientId, plan.id); router.push(`/patients/${patientId}/session`); }}><Play /> {plan.status === 'paused' ? 'Plan pausado' : hasCompletedSession ? 'Iniciar nueva sesión' : 'Iniciar sesión'}</button>}
      <small className="atal-local-save-copy">Tu progreso se guarda automáticamente.</small>
    </section>
