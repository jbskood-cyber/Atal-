...
export function PatientPortalPreviewScreen({ patientId }: { patientId: string }) {
  ...
  const store = useAtalStore((state) => ({ revision: state.updatedAt, sessions: state.sessions.filter((item) => item.patientId === patientId) }));
  const patient = useMemo(() => getPatientById(patientId), [patientId, store.revision]);
  const plan = useMemo(() => resolvePatientPlan(patientId), [patientId, store.revision]);
  ...
  const closeRestart = () => { setRestartOpen(false); window.setTimeout(() => restartTriggerRef.current?.focus(), 0); };
  useEffect(() => { if (!restartOpen) return; restartCancelRef.current?.focus(); const close = (event: KeyboardEvent) => { if (event.key === 'Escape') closeRestart(); }; window.addEventListener('keydown', close); return () => window.removeEventListener('keydown', close); }, [restartOpen]);
  ...
  const latestSession=store.sessions.find((item)=>item.planId===plan.id);const completed = stored ? Object.values(stored.exercises).filter((record) => record.result === 'completed').length : latestSession ? Object.values(latestSession.exercises).filter((record)=>record.result==='completed').length : 0;
  const restart = () => { clearSessionDraft(patientId, plan.id); setRestartOpen(false); router.push(`/patients/${patientId}/session`); };
