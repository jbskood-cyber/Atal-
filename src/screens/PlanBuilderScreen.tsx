...
export function PlanBuilderScreen() {
  ...
  return <AtalShell>{selecting ? <main className="atal-content atal-flow-page"><ExerciseSelector initialIds={selectedIds} onBack={() => setSelecting(false)} onConfirm={(ids) => { setSelectedIds(ids); setSelecting(false); }} /></main> : <main className="atal-content atal-flow-page">
    ...
    <form className="atal-clinical-form atal-plan-form" onSubmit={submit}>
      <fieldset><label className="atal-field atal-field--full"><span>Paciente</span>{patient ? <div className="atal-patient-picker"><Avatar name={patient.name} /><span className="atal-patient-picker__copy"><b>{patient.name}</b><small>{patient.diagnosis || patient.affectedArea || 'Seguimiento clínico'}</small></span><span className="atal-patient-picker__state"><i aria-hidden />Seleccionado</span><AppSelect label="Seleccionar paciente" value={patient.id} triggerLabel="Cambiar" className="atal-patient-change" options={patients.map((item) => ({value:item.id,label:item.name,detail:`${item.diagnosis || item.affectedArea || 'Seguimiento'} · ${item.status==='attention'?'Atención':'Activo'}`}))} onChange={setPatientId} /></div> : <button type="button" className="atal-patient-picker" onClick={() => router.push('/patients/new')}><Plus/><span><b>Crear paciente primero</b><small>No hay pacientes disponibles.</small></span><ChevronRight/></button>}</label>
