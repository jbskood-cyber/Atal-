...
export function ActivityScreen() {
  ...
  return <AtalShell><main className="atal-content atal-list-page atal-activity-page">
    ...
    <div className="atal-activity-feed">{entries.map((patient, index) => <button type="button" key={patient.id} onClick={() => router.push(`/patients/${patient.id}`)}><Avatar name={patient.name} /><span><b>{patient.name}</b><small>{view === 'tracking' ? patient.diagnosis : `${patient.plan} · Sesión ${index + 2}`}</small><em>{view === 'tracking' ? `Dolor ${2 + index % 5}/10　•　Adherencia ${patient.adherence}%` : index % 2 ? 'Comentario del paciente incluido' : 'Sesión enviada correctamente'}</em></span><time><CalendarDays />{index < 2 ? 'Hoy' : 'Ayer'}</time><i className={index % 3 === 1 ? 'is-attention' : ''} /><ChevronRight /></button>)}</div>
  </main></AtalShell>;
}
