...
function seedState(): AtalState {
  ...
  const events: ActivityEvent[] = patients.slice(0,4).map((patient, index) => ({ id: `seed-event-${patient.id}`, kind: 'patient_created', patientId: patient.id, title: 'Paciente', detail: patient.name, createdAt: new Date(Date.now() - index * 3600000).toISOString() }));
...
export function updatePatient(id:string, patch:Partial<PatientEntity>): PatientEntity | null { let result:PatientEntity|null=null; mutateAtalStore((draft)=>{const index=draft.patients.findIndex((item)=>item.id===id);if(index<0)throw new Error('Paciente no encontrado.');result={...draft.patients[index],...patch,id,updatedAt:now()};draft.patients[index]=result;addEvent(draft,{kind:'patient_updated',patientId:id,title:'Paciente actualizado',detail:result.name});}); return result; }
export function setPatientArchived(id:string,archived:boolean): PatientEntity | null {let result:PatientEntity|null=null;mutateAtalStore((draft)=>{const patient=draft.patients.find((item)=>item.id===id);if(!patient)throw new Error('Paciente no encontrado.');patient.status=archived?'archived':'active';patient.updatedAt=now();result=patient;addEvent(draft,{kind:archived?'patient_archived':'patient_restored',patientId:id,title:archived?'Paciente archivado':'Paciente restaurado',detail:patient.name});});return result;}
...
export function updatePlan(id:string,patch:Partial<PlanEntity>): PlanEntity | null {let result:PlanEntity|null=null;mutateAtalStore((draft)=>{const index=draft.plans.findIndex((item)=>item.id===id);if(index<0)throw new Error('Plan no encontrado.');result={...draft.plans[index],...patch,id,exerciseIds:patch.exerciseIds?[...new Set(patch.exerciseIds)]:draft.plans[index].exerciseIds,updatedAt:now()};draft.plans[index]=result;addEvent(draft,{kind:'plan_updated',patientId:result.patientId,planId:id,title:'Plan actualizado',detail:result.title});});return result;}
...
export function updateExercise(id:string,patch:Partial<ExerciseEntity>): ExerciseEntity | null {let result:ExerciseEntity|null=null;mutateAtalStore((draft)=>{const index=draft.exercises.findIndex((item)=>item.id===id);if(index<0)throw new Error('Ejercicio no encontrado.');result={...draft.exercises[index],...patch,id,updatedAt:now()};draft.exercises[index]=result;});return result;}
