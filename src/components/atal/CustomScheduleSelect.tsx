    ...
    {open && <div className="atal-select-overlay" onMouseDown={close}><section className="atal-custom-select-sheet" role="dialog" aria-modal="true" aria-label={label} onMouseDown={(event) => event.stopPropagation()}>
      ...
      {!custom ? <div className="atal-select-options">
        ...
        <div className="atal-custom-schedule-grid">
          ...
          <div className="atal-custom-unit">
            <span>Unidad</span>
            <div className="atal-custom-unit-options" role="group" aria-label="Unidad">
              {(mode === 'duration' ? ['días', 'semanas', 'meses'] : ['por día', 'por semana', 'por mes']).map((item) => (
                <button type="button" key={item} className={unit === item ? 'is-selected' : ''} aria-pressed={unit === item} onClick={() => setUnit(item)}>
                  <span>{item}</span>{unit === item && <Check aria-hidden="true" />}
                </button>
              ))}
            </div>
          </div>
